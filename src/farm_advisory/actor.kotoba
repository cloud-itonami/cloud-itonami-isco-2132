(ns farm-advisory.actor
  "AdvisoryActor — the ISCO-08 2132 farming, forestry and fisheries
  adviser actor as a `langgraph.graph/state-graph` (per ADR-2607011000
  / CLAUDE.md Actors section). One graph run = one advisory request
  ...
  The unconditional invariant: the AdvisoryAdvisor can never
  directly commit a record or dispatch an action the
  AdvisoryGovernor refuses — every commit-record! call is gated
  behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [farm-advisory.advisor :as advisor]
            [farm-advisory.governor :as governor]
            [farm-advisory.store :as store]))

(defn build-graph
  "Build a compiled AdvisoryActor graph. `store` implements
  `farm-advisory.store/Store`. `advisor` implements
  `farm-advisory.advisor/Advisor` (defaults to `mock-advisor`).
...
(defn run-request!
  "Run one advisory request to completion or interrupt. `thread-id`
